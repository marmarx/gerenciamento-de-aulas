...
const addDays = (add, date = new Date()) => {
  const dISO = dateISO(date)
  const d = parseDate(dISO)
  const newDate = d.setDate(d.getDate() + add)
...
// Label functions
const dateLabel = (dataISO, compare = new Date()) => {  // YYYY-MM-DD -> "26 de ago" OR "26 de ago de 2024"
  const date = parseDate(dataISO)
  const sameYear = date.getFullYear() === new Date(compare).getFullYear()
...
const shortDateLabel = (dataISO, compare = new Date()) => { //2025-10-25 -> '25/10' or '25/10/2024'
  const date = parseDate(dataISO);
  const sameYear = date.getFullYear() === new Date(compare).getFullYear()
...
const formatDur = (d) => {
  ...
  const s = seconds || 0 ? `${seconds}s` : '0s'  // always show 0s when needed
