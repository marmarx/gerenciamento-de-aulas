...
export function useEventDefaults(event) {
  ...
  // check if current event overlaps any other existing event
  const eventOverlaps = (currentEvent) => {
    if (!currentEvent.date || !currentEvent.time || !currentEvent.dateEnd || !currentEvent.timeEnd) return false

    const startA = parseDate(currentEvent.date, currentEvent.time)
    const endA   = parseDate(currentEvent.dateEnd, currentEvent.timeEnd)

    return dataStore.data.events.some(ev => {
      if (ev.id_event === currentEvent.id_event) return false     // ignore itself
      if (ev.status === 'canceled') return false                  // ignore canceled events
      if (ev.date !== currentEvent.date) return false             // must match same day to be considered overlapping
  ...
  const overlapWarning = computed(() => eventOverlaps(event))

  return { finishEvent, initialUpdate, overlapWarning }
}