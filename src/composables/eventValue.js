...
const eventValue = (id) => {
  ...
  const e = dataStore.sortedEvents.find(ev => ev.id_event === id)
  if (!e) return 0
  if (e.experimental || e.deleted) return 0

  let cancelationFee = 100
  
  const student = dataStore.sortedStudents.find(s => s.id_student === e.id_student)
  ...
  const duration = Number(e.duration) || Number(dataStore.sortedConfig.duration) // 0 is not valid
