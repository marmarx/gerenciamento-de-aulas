...
// update student's name on every event and payment in case it is changed
const updateStudentName = (id, name) => {
    dataStore.sortedEvents.forEach(e => { if (e.id_student === id) e.student_name = name })
  dataStore.sortedPayments.forEach(p => { if (p.id_student === id) p.student_name = name })
}

let debounceTimer = null  // avoid saving multiple changes to localStorage and increases performance

watch(() => student.student_name, (newName) => {
  if (!newName) return

  clearTimeout(debounceTimer)
  debounceTimer = setTimeout(() => updateStudentName(student.id_student, newName), 500)
})
</script>
  ...
  <div v-if="student" class="section">
    ...
    <div class="halfContainer">
      ...
      <div class="half">
        ...
        <p v-if="student.student_phone">
          <b>Telefone:</b> <alink :title="`Abrir whatsapp de ${student.student_name}`" :href="whatsappLink(student.student_phone)">{{student.student_phone}}</alink>
        </p>
        ...
        <div v-if="student.scholl || student.year">
          ...
          <p v-if="student.parent_phone">
            <b>Telefone:</b> <alink :title="`Abrir whatsapp de ${student.parent}`" :href="whatsappLink(student.parent_phone)">{{student.parent_phone}}</alink>
          </p>
          <p v-if="student.parent_2"><b>Responsável:</b> {{student.parent_2}}</p>
          <p v-if="student.parent_2_phone">
            <b>Telefone:</b> <alink :title="`Abrir whatsapp de ${student.parent_2}`" :href="whatsappLink(student.parent_2_phone)">{{student.parent_2_phone}}</alink>
          </p>
          <hr/>
        </div>

        <div v-if="student.address || student.meeting || student.scholl || student.year">
          <p v-if="student.address">
            <b>Endereço:</b> <alink title="Navegar para endereço" :href="mapsLink(student.address)">{{student.address}}</alink>
          </p>
          <p v-if="student.meeting">
            <b>Aula virtual:</b> <alink title="Abrir vídeo chamada" :href="student.meeting">Vídeo chamada</alink>
          </p>
