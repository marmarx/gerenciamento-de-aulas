  ...
  <div class="section">
    ...
    <div class="container">
      <inputToggle v-model="dataStore.data.config.canceledOnReport">
        ...
        <template #helpText>Aulas canceladas {{ dataStore.sortedConfig.canceledOnReport ? '' : 'não ' }} são exibidos no relatório.</template>
      </inputToggle>
      <inputToggle v-model="dataStore.data.config.paymentsOnReport">
        <template #title>{{ dataStore.sortedConfig.paymentsOnReport ? 'M' : 'Não m' }}ostrar pagamentos</template>
        <template #helpText>Pagamentos {{ dataStore.sortedConfig.paymentsOnReport ? '' : 'não ' }} são exibidos nos relatórios.</template>
      </inputToggle>
      <hr/>
    </div>

    <div v-if="dataStore.selectedStudent && filterStart && filterEnd" v-html="report" class="report-content"></div>
...
<style scoped>
.report-content, .report-content * { line-height: 2em; -webkit-user-select: all; user-select: all }
</style>