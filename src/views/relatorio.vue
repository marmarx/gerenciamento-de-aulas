...
const reportContent = computed(() => {
  const details = `<b>Aluno(a)</b>: ${dataStore.student.student_name}<br><br><b>Período</b>: de ${(filterStart)} à ${shortDateLabel(filterEnd)}<br><br>`
...
const copyToClipboard = () => {
  navigator.clipboard.writeText(reportContent.value)
    .then(() => toastShow('Copiado!','Relatório copiado para a área de transferência') )
    .catch(err => {
