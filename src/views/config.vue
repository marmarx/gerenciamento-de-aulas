  ...
  <div class="section">
    ...
    <div class="flexContainer mw500">
      ...
      <inputHelp id="duration" placeholder="Horas" :numberDefs="{min: .25, step: .25}" v-model.number="dataStore.data.config.duration">
        ...
        <template #helpText>
          As aulas terão {{ formatDuration(dataStore.data.config.duration) }} de duração padrão, caso não especificado individualmente em cada evento
        </template>
      </inputHelp>
      ...
      <inputHelp id="cost" :placeholder="`${currency(0).slice(0,2)}${dataStore.data.config.variableCost?'/h':''}`" :numberDefs="{min: 0, step: .5}" v-model.number="dataStore.data.config.cost">
        ...
        <template #helpText>
          O valor {{dataStore.data.config.variableCost ? 'hora' : 'da'}} aula de {{ currency(dataStore.data.config.cost) }} será utilizado como padrão
        </template>
      </inputHelp>
    ...
    <div class="flexContainer mw500">
      ...
      <inputToggle v-model="dataStore.data.config.chargeCancelations">
        ...
        <template #helpText>
          Aulas canceladas {{ dataStore.data.config.chargeCancelations ? 'serão cobradas conforme o período de gratuidade e a taxa de cancelamento abaixo' : 'não serão cobradas' }} por padrão
          </template>
      </inputToggle>
      ...
      <inputHelp id="freeBefore" :if="dataStore.data.config.chargeCancelations" placeholder="Horas" :numberDefs="{min: 0, step: .25}" v-model.number="dataStore.data.config.freeCancelationBefore">
        ...
        <template #helpText>
          Não serão cobrados cancelamentos que ocorram {{ dataStore.data.config.freeCancelationBefore ? `com no mínimo ${formatDuration(dataStore.data.config.freeCancelationBefore)} de antecedência` : 'até o horário da aula' }} por padrão
        </template>
      </inputHelp>
    ...
    <div class="flexContainer mw500">
      ...
      <inputToggle v-if="permissionGranted" v-model="dataStore.data.config.notifyBirthday">
        ...
        <template #helpText>Notificações {{ dataStore.data.config.notifyBirthday ? '' : 'não ' }}dos aniversários de cada aluno serão enviadas.</template>
      </inputToggle>

      <inputToggle v-if="permissionGranted && dataStore.data.config.notifyBirthday" v-model="dataStore.data.config.notBirthDayBefore">
        <template #title>Aniversários no dia anterior</template>
        <template #helpText>Notificações de aniversários serão enviadas às 9 horas da manhã do dia {{ dataStore.data.config.notBirthDayBefore ? 'anterior ao' : 'do' }} aniversário.</template>
      </inputToggle>
