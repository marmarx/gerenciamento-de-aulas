...
// console.log(event.value.id_event, student.value.id_student, student.value.student_name)

const startDate = computed(() => parseDate(event.value.date, event.value.time).getTime())
// const startDate = computed(() => parseDate(event.value.dateEnd, event.value.timeEnd).getTime())
