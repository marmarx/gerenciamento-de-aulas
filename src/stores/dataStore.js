...
const newData = () => (
  {
    ...
    config: {
      ...
      color: '#44289e', canceledOnReport: true, advancedOptions: false, notifyBirthday: true, lastRefreshDate: null, // user interface settings
      ...defaults
    }
  }
)
...
export const useDataStore = defineStore(storageTitle, () => {
  ...
  const clearStorage = async () => {
    ...
    const ok = await toastShow('Apagar todos os dados?', 'Esta ação não pode ser desfeita.', true, 10000)
    if(!ok) return
