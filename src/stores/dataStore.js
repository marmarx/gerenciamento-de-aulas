...
const newData = () => (
  {
    ...
    config: {
      ...
      canceledOnReport: true, paymentsOnReport: false, advancedOptions: false, notifyBirthday: true, notBirthDayBefore: true, lastRefreshDate: null, // user interface settings
      ...defaults
    }
  }
)
...
export const useDataStore = defineStore(storageTitle, () => {
  ...
  const loadStorage = () => {
    try {
      // console.log('[dataStore] Data loaded from localStorage')
      return JSON.parse(localStorage.getItem(storageTitle)) || newData()
    }
    catch(error) {
      // console.error('[dataStore] Error parsing localStorage!\n',error)
      return newData()
    }
  }
