...
const previousDates = arr => arr.filter(e => parseDate(e.date) < parseDate(startDate.value))
...
const previousEvents    = computed(() => previousDates(dataStore.sortedEvents    || []))
const previousChargable = computed(() => previousDates(dataStore.chargableEvents || []))
const previousDone      = computed(() => previousDates(dataStore.doneEvents      || []))
const previousPayments  = computed(() => previousDates(dataStore.sortedPayments  || []))

export {
  filterStart, filterEnd,
  startDate, endDate,
  eventsInRange, chargableInRange, doneInRange, paymentsInRange,
  previousEvents, previousChargable, previousDone, previousPayments
}