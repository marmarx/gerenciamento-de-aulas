...
export const useNotificationStore = defineStore('notificationStore', () => {
  ...
  const setupNotificationWatcher = () => {
    ...
    watch(
      () => CONFIG_FIELDS.map(key => dataStore.data.config[key]),
      async (newValues, oldValues) => {
        ...
        if (!notifyBirthday) {
          dataStore.data.students.forEach(s => { if (s.dob) cancelNotificationById(s.id_student) })
          return
        }

        dataStore.data.students.forEach(s => { if (s.dob) markBirthdayDirty(s.id_student) })
      },
      { deep: false }
    )
